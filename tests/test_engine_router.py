...
class ModelRouterEngineTests(unittest.TestCase):
    ...
    def test_scheduler_tracks_queued_work_separately_from_runtime_inflight(self) -> None:
        ...
        self.assertEqual(payload["fairness"]["rejected_per_key_limit"], 0)
        self.assertEqual(payload["fairness"]["rejected_executor_limit"], 0)
        self.assertEqual(len(payload["fairness"]["keys"]), 1)
        self.assertIsNone(payload["fairness"]["keys"][0]["fairness_key"])
        self.assertEqual(payload["fairness"]["keys"][0]["pending"], 1)
        self.assertEqual(payload["fairness"]["keys"][0]["active"], 1)
        self.assertEqual(payload["fairness"]["keys"][0]["weight"], 1.0)
        self.assertGreaterEqual(payload["fairness"]["keys"][0]["score"], 0.0)
