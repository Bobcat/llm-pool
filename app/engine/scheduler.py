...
class _FairPendingQueue:
    ...
    def rejected_count(self) -> int:
        return self._rejected_per_key_limit + self._rejected_executor_limit
    ...
    def complete(self, job: SchedulerJob, *, service_ms: float, now: float) -> None:
        ...
        if state is None or state.active_started_at.pop(job.job_id, None) is None:
            return
        state.virtual_service_ms += max(0.0, service_ms) / self._weight(job.fairness_key)
    ...
    def _score(self, state: _FairnessState, now: float) -> float:
    ...
    def _mark_idle_if_needed(self, state: _FairnessState, now: float) -> None:
...
class LoadedModelExecutor:
    ...
    def snapshot(self) -> ExecutorSnapshot:
        ...
        with self._cond:
            fairness_keys = self._pending_queue.snapshots(now=time.perf_counter())
            ...
            with self._cond:
                while True:
                    if (
                        ...
                        and self._runtime_inflight_locked() == 0
                    ):
                        return
                    selected_replica = self._pick_replica_locked()
                    if self._pending_queue.pending_count > 0 and selected_replica is not None:
...
class RuntimeScheduler:
    def __init__(self, fairness_settings: FairnessSettings | None = None) -> None:
        self._fairness_settings = fairness_settings or FairnessSettings()
